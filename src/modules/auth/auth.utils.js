...
import { createSession, deleteByToken, deleteByUserId } from '../token/token.service.js';
...
export const logoutService = async ({ refreshToken, userId, findUserById }) => {
  if (!userId) {
    throw AppError.validationError([{ field: 'userId', message: 'UserId is required' }]);
  }
