...
export const create = async (body) => {
  return await executeTransaction(async (client) => {
    if (body.parent_id) {
      const parent = await findCategoryById(body.parent_id, client);
...
export const getAll = async (query) => {
  ...
  if (!allowedSortFields.includes(sortBy)) {
    throw AppError.badRequest(`'sort' must be one of: ${allowedSortFields.join(', ')}`);
  }

  const allowedOrders = ['asc', 'desc'];
  if (!allowedOrders.includes(orderBy)) {
    throw AppError.badRequest(`'order' must be one of: ${allowedOrders.join(' or ')}`);
  }

  const startTime = process.hrtime.bigint();
  const { categories, totalFilteredCount } = await CategoryModel.findAllWithPagination(
    search,
    skip,
    limit,
    sortBy,
...
export const edit = async (categoryId, updates) => {
  return await executeTransaction(async (client) => {
    ...
    if (name) {
      ...
      const existing = await CategoryModel.findByNameOrSlug(name, newSlug, client);
    ...
    const updatedCategory = await CategoryModel.update(categoryId, payload, client);
...
export const getWithDetail = async (slug) => {
  if (!slug || typeof slug !== 'string' || slug.trim() === '') {
    throw AppError.badRequest('Slug parameter is required', 'Missing or invalid slug');
  }

  return await executeTransaction(async (client) => {
    const category = await CategoryModel.findByNameOrSlug(undefined, slug, client);
    if (!category) throw AppError.notFound('Category');

    const ancestors = category.parent_id ? await getAncestors(category.parent_id, client) : [];
    ...
    const inherited_attributes = await getInheritedAttributes(category.id, client);
...
export const addAttributeToCategory = async (categoryId, newAttr) => {
  return await executeTransaction(async (client) => {
    ...
    const rules = Array.isArray(category.attribute_rules) ? category.attribute_rules : [];
    ...
    const data = await CategoryModel.update(categoryId, { attribute_rules: updatedRules }, client);

    return { success: true, message: 'Attribute added', data };
  });
};

export const updateAttributeInCategory = async (categoryId, attrName, updates) => {
  return await executeTransaction(async (client) => {
    const category = await findCategoryById(categoryId, client);
    if (!category) throw AppError.notFound('Category');
    ...
    const data = await CategoryModel.update(categoryId, { attribute_rules: rules }, client);
...
export const deleteAttributeFromCategory = async (categoryId, attrName) => {
  return await executeTransaction(async (client) => {
    ...
    const data = await CategoryModel.update(categoryId, { attribute_rules: filteredRules }, client);
...
// Shared Servies:
export const findCategoryById = async (categoryId, client = prisma) => {
  const category = await CategoryModel.findById(categoryId, client);
  return category;
};

export const getCategoryLineage = async (categoryId, client) => {
  return await CategoryModel.fetchAncestry(categoryId, client);
};
